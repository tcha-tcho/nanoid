...
var ModelDocument = function (model_type, schema, views) {
  ...
  self.load = function (model_data) { 
    ...
    model.dateCreated = new Date(model_data.dateCreated);
    model.lastUpdated = new Date(model_data.lastUpdated);
...
Model.load = function () {
  ...
  async.forEach(Object.keys(defined_models), function (model_type, done_cb) {
    var model_document = defined_models[model_type]
    db.insert( {"views": model_document.views }, '_design/'+model_type, function (err, res) {
      if (err) {
        return done_cb(err)
      }
      done_cb();
